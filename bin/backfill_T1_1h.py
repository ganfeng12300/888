...
UNI=os.path.join(ROOT,'data','universe','T1.txt')
BASE="https://fapi.binance.com/fapi/v1/klines?symbol={sym}&interval=1h&limit=1000"
def get(u):
    ctx=ssl.create_default_context()
    with urllib.request.urlopen(u,timeout=20,context=ctx) as r:
        return json.loads(r.read().decode())
def rebuild(con,sym,rows):
    tbl=f"{sym}_1h"
...
for s in syms:
    try:
        ...
        if isinstance(rows,list) and rows:
            rebuild(con,s,rows); print("ok",s,len(rows))
